...
function load() {
  ...
  if (import.meta.env.DEV) {
    log('hybrid-router', 'Registered observer');
  }

  document.body.querySelectorAll('a').forEach(registerAnchor);
  if (import.meta.env.DEV) {
    log('hybrid-router', 'Registered anchor tags synchronously');
  }
  ...
  if (import.meta.env.DEV) {
    log('hybrid-router', 'Observing anchor tags');
  }
}
...
export function setup() {
  setupEvents();
  if (import.meta.env.DEV) {
    log('hybrid-router', 'Registered events');
  }
